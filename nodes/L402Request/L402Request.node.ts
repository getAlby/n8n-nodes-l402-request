    ...
    for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
      ...
      if (responseFormat === 'file') {
        ...
        if (fullResponse) {
          ...
          returnItem['body'] = toText(response.body);
          returnItem['headers'] = response.headers;
          returnItem['statusCode'] = response.status;
          returnItem['statusMessage'] = response.statusText;
